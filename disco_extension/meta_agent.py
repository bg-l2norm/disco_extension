...
def unroll_jittable_actor(
    ...
    rollout_len: int,
    actor_step_fn: Any,
) -> Tuple[types.ActorRollout, Any, types.EnvironmentTimestep, Any]:
    """Helper function to unroll the actor in a jittable environment."""
...
class MetaTrainAgent:
    """Meta training agent bundles together all meta training-related functionality."""
    ...
    def __init__(
        ...
        """
        Args:
            env: An instantiated environment.
            agent_settings: Configuration for the agent.
            value_fn_config: Configuration for the value function.
            axis_name: Axis name for pmap/pmean, if applicable.
        """
        self.env = env
    ...
    def init_state(self, rng_key: chex.PRNGKey, batch_size: int) -> MetaTrainState:
        ...
        Args:
            rng_key: Random key.
            batch_size: The batch size used for the dummy observation (must match env batch size).
        """
...
def meta_update(
    ...
    rng: chex.PRNGKey,
    meta_opt: optax.GradientTransformation,
    num_inner_steps: int,
    rollout_len: int,
    axis_name: Optional[str] = None,
):
    ...
    Args:
        update_rule_params: Current meta parameters.
        meta_opt_state: Optimizer state for meta parameters.
        agents_states: List of current states for each agent.
        agents: List of MetaTrainAgent instances.
        rng: Random key.
        meta_opt: The optax optimizer instance.
        num_inner_steps: Number of inner steps to unroll.
        rollout_len: Length of each rollout.
        axis_name: Axis name for parallelization.
    """
    ...
    meta_update_val, meta_opt_state = meta_opt.update(
        avg_meta_gradient, meta_opt_state
    )
    update_rule_params = optax.apply_updates(update_rule_params, meta_update_val)

    meta_log['meta_grad_norm'] = optax.global_norm(avg_meta_gradient)
    meta_log['meta_up_norm'] = optax.global_norm(meta_update_val)
